...
namespace nu
{
	...
	void RigidBodyPhysicsComponent::Update(float dt)
	{
		...
		m_angular_velocity += m_angular_acceleration * dt;
		m_angular_velocity *= 1.0f / ((1.0f) + m_angularDamping * dt);
	...
	{
		m_angular_acceleration += torque / m_mass;
	}
	...
	void RigidBodyPhysicsComponent::Read(const json::value_t& value)
	{
		...
		JSON_READ_NAME(value, "angular_velocity", m_angular_velocity);
	}
}